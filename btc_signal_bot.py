...
SIGNALS_LOG_FILE = "btc_signals_log.csv"
PAPER_TRADES_FILE = "btc_paper_trades.csv"
...
MAX_RECENT_MOVE_PERCENT = 0.90

# فعلاً ADX و Volume فقط هشدار و امتیاز هستند، نه شرط قطعی ورود.
# اگر بعداً خواستیم ربات BTC سخت‌گیرتر شود، این دو را True می‌کنیم.
...
def is_important_market_event(event):
...
def get_news_risk():
    ...
    for event in events:
        ...
        if not is_important_market_event(event):
            continue
    ...
    if not relevant_events:
        return {
            "risk": "LOW",
            "trade_allowed": True,
            "summary": "خبر مهم اقتصادی آمریکا در بازه امروز/فردا پیدا نشد.",
            "events": []
        }
...
def is_duplicate_recent(now_utc, technical_signal, final_signal, block_reason):
    ...
    for row in reversed(rows):
        row_time = parse_log_timestamp(row.get("timestamp_turkey", ""))
...
if __name__ == "__main__":
    try:
        ...
        error_message = f"❌ BTC Signal Bot Error:\n{str(e)}"
